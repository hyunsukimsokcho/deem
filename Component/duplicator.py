...
class Duplicator:
    ...
    def parallel_duplicate(self, n, k):
        ...
        with concurrent.futures.ProcessPoolExecutor(max_workers=5) as executor:
            ...
            for i in range(n-1):
                batch_jobs.append(executor.submit(self.add_raw_duplicate, self.train_data[i*batch_size:(i+1)*batch_size], k, isParallel=True))
            batch_jobs.append(executor.submit(self.add_raw_duplicate, self.train_data[(n-1)*batch_size:], k, isParallel=True))
            for job in concurrent.futures.as_completed(batch_jobs):
                if job.cancelled():
                    print("[PARALLEL_DUPLICATE: JOB cancelled]: Check what's causing abnormal behavior")
                    continue
                elif job.done():
    ...
    def add_raw_duplicate(self, data, k, isParallel=False):
        """
        @description
            Generate duplicate of each tuple according to zipfian dist.

        @params
            data: data frame to add raw duplicate
            k: maximum number of duplicates to generate according to zipfian dist. (k>1)
            (As k increases, number of duplicates decreases. k=4 brings about 10% duplicates; HEURISTIC)
